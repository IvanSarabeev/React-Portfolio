...
const ProjectLayout = ({ item, currentImg }) => {
  return (
    <>
      <AnimatePresence>
        <section className="my-4 flex flex-col items-center justify-center gap-x-4 shadow-none sm:gap-x-6 md:gap-x-8 lg:gap-x-10 lg:p-6 lg:shadow-2xl">
          <motion.h3
            variants={context}
            initial="initial"
            animate="animate"
          ...
          <article className="flex w-full flex-col space-y-3 md:w-2/3">
            ...
            <span className="flexCenter mt-2 gap-x-1 sm:gap-x-2 md:gap-x-3 lg:gap-x-4">
              <Link to={item.demoLink} target="_blank" className="project-link">
                <Button type="button" className="project-btn">
                  <IconArrowRight height={24} width={24} />
                  Live Demo
                </Button>
              </Link>
              <Link to={item.codeLink} target="_blank" className="project-link">
                <Button type="button" className="project-btn">
                  <UilGithub className="h-4 w-4 md:h-6 md:w-6" />
                  Source Code
                </Button>
              </Link>
            </span>
