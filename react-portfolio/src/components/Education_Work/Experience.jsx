  ...
  return (
    ...
    >
      <div className="relative flex flex-col rounded-xl p-6 hover:ring-2 hover:ring-neutral-400/30">
        <h4 className="my-2 break-words font-paragraph text-sm italic leading-4 text-[#2686976] md:text-base">
          {exp.period}
        </h4>
        <h3 className="mt-3 font-pageheading text-lg font-semibold capitalize sm:text-xl md:mt-2 md:text-[22px]">
          {exp.company}
        </h3>
        <span className="my-2 flex items-start gap-x-1 font-paragraph capitalize md:flex-row md:items-center md:gap-x-2">
          <h5 className="leading-6 text-[#2686976] md:leading-8">Position:</h5>
          <h6 className="font-paragraph font-semibold">{exp.title}</h6>
        </span>
        <img
          src={exp.icon}
          alt={exp.title}
